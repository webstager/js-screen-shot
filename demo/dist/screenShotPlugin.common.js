...
function setSelectedClassName(mouseEvent, index, isOption) {
  ...
  var path = mouseEvent.path || mouseEvent.composedPath && mouseEvent.composedPath(); // 获取div下的所有子元素

  var nodes = path[1].children;

  for (var i = 0; i < nodes.length; i++) {
    var item = nodes[i]; // 如果工具栏中已经有选中的class则将其移除
...
function takeOutHistory() {
  ...
  data.setUndoClickNum(data.getUndoClickNum() + 1); // 历史记录已取完，禁用撤回按钮点击
...
var draggingTrim = false; // 裁剪框拖拽状态

var dragging = false; // 截图工具栏点击状态

var toolClickStatus = false; // 当前选择的颜色

var selectedColor = "#F53340"; // 当前点击的工具栏名称

var InitData_toolName = ""; //  当前选择的画笔大小

var penSize = 2; // 裁剪框顶点边框直径大小

var InitData_borderSize = 10; // 撤销点击次数

var undoClickNum = 0; // 画笔历史记录

var InitData_history = []; // 文本输入工具栏点击状态

var textClickStatus = false; // 裁剪框位置参数
...
var InitData_screenShortController = null; // 获取截图工具栏容器dom

var toolController = null; // 获取文本输入区域dom

var InitData_textInputController = null; // 截图工具栏画笔选择dom
...
var undoController = null; // 屏幕截图容器

var screenShortImageController = null; // 数据初始化标识
...
var InitData_InitData = /*#__PURE__*/function () {
  ...
  _createClass(InitData, [{
    ...
    value: function setOptionPosition(position) {
      ...
      optionIcoController = this.getOptionIcoController();
      optionController = this.getOptionController();
      if (optionIcoController == null || optionController == null) return; // 修改位置
    ...
    value: function destroyDOM() {
      if (InitData_screenShortController == null || toolController == null || optionIcoController == null || optionController == null || InitData_textInputController == null) return; // 销毁dom
...
function saveCanvasToImage(context, startX, startY, width, height) {
  // 获取裁剪框区域图片信息
  var img = context.getImageData(startX, startY, width, height); // 创建canvas标签，用于存放裁剪区域的图片

  var canvas = document.createElement("canvas");
  canvas.width = width;
  canvas.height = height; // 获取裁剪框区域画布
  ...
  if (imgContext) {
    ...
    var a = document.createElement("a"); // 获取图片

    a.href = canvas.toDataURL("png"); // 下载图片
...
function saveCanvasToBase64(context, startX, startY, width, height) {
  ...
  if (imgContext) {
    // 将图片放进canvas中
    imgContext.putImageData(img, 0, 0); // 将图片放进剪贴板中

    canvas.toBlob(function (blob) {
      var clipboardItem = new window.ClipboardItem(_defineProperty({}, blob.type, blob));
...
function getCanvasImgData(isSave) {
  ...
  var data = new InitData_InitData();
  var screenShortCanvas = (_data$getScreenShortC = data.getScreenShortController()) === null || _data$getScreenShortC === void 0 ? void 0 : _data$getScreenShortC.getContext("2d"); // 获取裁剪区域位置信息
  ...
  if (screenShortCanvas) {
    if (isSave) {
      ...
      if (width > 0 && height > 0) {
        // 考虑左上往右下拉区域的情况
        saveCanvasToImage(screenShortCanvas, startX + data.getBorderSize() / 2, startY + data.getBorderSize() / 2, width - data.getBorderSize(), height - data.getBorderSize());
      } else if (width < 0 && height < 0) {
        // 考虑右下往左上拉区域的情况
        saveCanvasToImage(screenShortCanvas, startX + width + data.getBorderSize() / 2, startY + height + data.getBorderSize() / 2, Math.abs(width) - data.getBorderSize(), Math.abs(height) - data.getBorderSize());
      } else if (width > 0 && height < 0) {
        // 考虑左下往右上拉区域的情况
        saveCanvasToImage(screenShortCanvas, startX + data.getBorderSize() / 2, startY + height + data.getBorderSize() / 2, width - data.getBorderSize(), Math.abs(height) - data.getBorderSize());
      } else if (width < 0 && height > 0) {
        // 考虑右上往左下拉区域的情况
        saveCanvasToImage(screenShortCanvas, startX + width + data.getBorderSize() / 2, startY + data.getBorderSize() / 2, Math.abs(width) - data.getBorderSize(), height - data.getBorderSize());
      }
    } else {
      // 将canvas转为base64
      if (width > 0 && height > 0) {
        // 考虑左上往右下拉区域的情况
        base64 = saveCanvasToBase64(screenShortCanvas, startX + data.getBorderSize() / 2, startY + data.getBorderSize() / 2, width - data.getBorderSize(), height - data.getBorderSize());
      } else if (width < 0 && height < 0) {
        // 考虑右下往左上拉区域的情况
        base64 = saveCanvasToBase64(screenShortCanvas, startX + width + data.getBorderSize() / 2, startY + height + data.getBorderSize() / 2, Math.abs(width) - data.getBorderSize(), Math.abs(height) - data.getBorderSize());
      } else if (width > 0 && height < 0) {
        // 考虑左下往右上拉区域的情况
        base64 = saveCanvasToBase64(screenShortCanvas, startX + data.getBorderSize() / 2, startY + height + data.getBorderSize() / 2, width - data.getBorderSize(), Math.abs(height) - data.getBorderSize());
      } else if (width < 0 && height > 0) {
        // 考虑右上往左下拉区域的情况
        base64 = saveCanvasToBase64(screenShortCanvas, startX + width + data.getBorderSize() / 2, startY + data.getBorderSize() / 2, Math.abs(width) - data.getBorderSize(), height - data.getBorderSize());
      }
    }
  } // 销毁组件
...
var enableWebRtc = true; // 数据初始化标识

var PlugInParameters_initStatus = false; // 画布宽高
...
var PlugInParameters_PlugInParameters = /*#__PURE__*/function () {
  function PlugInParameters() {
    ...
    if (PlugInParameters_initStatus) {
      enableWebRtc = true; // 初始化完成设置其值为false
...
function drawCutOutBox(mouseX, mouseY, width, height, context, borderSize, controller, imageController) {
  ...
  var plugInParameters = new PlugInParameters_PlugInParameters(); // 画布、图片不存在则return

  if (!canvasWidth || !canvasHeight || !imageController || !controller) return; // 清除画布

  context.clearRect(0, 0, canvasWidth, canvasHeight); // 绘制蒙层

  context.save();
  context.fillStyle = "rgba(0, 0, 0, .6)";
  context.fillRect(0, 0, canvasWidth, canvasHeight); // 将蒙层凿开

  context.globalCompositeOperation = "source-atop"; // 裁剪选择框

  context.clearRect(mouseX, mouseY, width, height); // 绘制8个边框像素点并保存坐标信息以及事件参数

  context.globalCompositeOperation = "source-over";
  context.fillStyle = "#2CABFF"; // 是否绘制裁剪框的8个像素点

  if (drawBorders) {
    // 像素点大小
    var size = borderSize; // 绘制像素点
  ...
  context.restore(); // 使用drawImage将图片绘制到蒙层下方

  context.save(); // 获取图片canvas容器的宽高信息
...
function toolClickEvent(toolName, index, mouseEvent, completeCallback, closeCallback) {
  ...
  if (screenShortController == null || ScreenShortImageController == null) return; // 获取canvas容器

  var screenShortCanvas = screenShortController.getContext("2d"); // 工具栏尚未点击，当前属于首次点击，重新绘制一个无像素点的裁剪框

  if (!data.getToolClickStatus()) {
    // 获取裁剪框位置信息
    var cutBoxPosition = data.getCutOutBoxPosition(); // 开始绘制无像素点裁剪框
  ...
  data.setToolName(toolName); // 为当前点击项添加选中时的class名

  setSelectedClassName(mouseEvent, index, false);

  if (toolName != "text") {
    // 显示画笔选择工具栏
    data.setOptionStatus(true); // 设置画笔选择工具栏位置
  ...
  data.setDraggingTrim(false); // 保存图片

  if (toolName == "save") {
    getCanvasImgData(true); // 销毁组件
  ...
  if (toolName == "confirm") {
    var base64 = getCanvasImgData(false); // 触发回调函数，截图数据回传给插件调用者
...
function setBrushSize(size, index, mouseEvent) {
  var data = new InitData_InitData(); // 为当前点击项添加选中时的class名
...
function selectColor() {
  var data = new InitData_InitData(); // 显示颜色选择面板
...
function getColor(index) {
  ...
  data.setSelectedColor(currentColor); // 隐藏颜色选择面板
...
var CreateDom_CreateDom = /*#__PURE__*/function () {
  function CreateDom(options) {
    ...
    this.closeCallback = options === null || options === void 0 ? void 0 : options.closeCallback; // 成功回调函数不存在则设置一个默认的
    ...
    this.setAllControllerId(); // 为画笔绘制选项角标设置class

    this.setOptionIcoClassName();
    this.toolbar = Toolbar; // 渲染工具栏

    this.setToolBarIco(); // 渲染画笔相关选项

    this.setBrushSelectPanel(); // 渲染文本输入

    this.setTextInputPanel(); // 渲染页面

    this.setDomToBody(); // 隐藏所有dom
  ...
  _createClass(CreateDom, [{
    ...
    value: function setToolBarIco() {
      ...
      var _loop = function _loop(i) {
        ...
        var itemPanel = document.createElement("div"); // 撤销按钮单独处理
    ...
    value: function setBrushSelectPanel() {
      ...
      rightPanel.className = "right-panel"; // 创建颜色选择容器
      ...
      rightPanel.id = "rightPanel"; // 创建颜色下拉箭头选择容器
      ...
      rightPanel.appendChild(pullDownArrow); // 向画笔绘制选项容器追加画笔选择和颜色显示容器
    ...
    value: function setTextInputPanel() {
      // 让div可编辑
      this.textInputController.contentEditable = "true"; // 关闭拼写检查
...
function drawMasking(context) {
  // 清除画布
  context.clearRect(0, 0, window.innerWidth, window.innerHeight); // 绘制蒙层

  context.save();
  context.fillStyle = "rgba(0, 0, 0, .6)";
  context.fillRect(0, 0, window.innerWidth, window.innerHeight); // 绘制结束
...
function drawText(text, mouseX, mouseY, color, fontSize, context) {
  // 开始绘制
  context.save();
  context.lineWidth = 1; // 设置字体颜色

  context.fillStyle = color;
  context.textBaseline = "middle";
  context.font = "bold ".concat(fontSize, "px \u5FAE\u8F6F\u96C5\u9ED1");
  context.fillText(text, mouseX, mouseY); // 结束绘制
...
function drawRectangle(mouseX, mouseY, width, height, color, borderWidth, context, controller, imageController) {
  ...
  context.strokeStyle = color; // 设置边框大小
...
function drawCircle(context, mouseX, mouseY, mouseStartX, mouseStartY, borderWidth, color) {
  ...
  var endY = mouseY >= mouseStartY ? mouseY : mouseStartY; // 计算圆的半径

  var radiusX = (endX - startX) * 0.5;
  var radiusY = (endY - startY) * 0.5; // 计算圆心的x、y坐标

  var centerX = startX + radiusX;
  var centerY = startY + radiusY; // 开始绘制
...
function drawLineArrow(context, mouseStartX, mouseStartY, mouseX, mouseY, theta, slashLength, borderWidth, color) {
  ...
  context.save();
  context.beginPath(); // P3的坐标位置

  var arrowX = mouseStartX - topX,
      arrowY = mouseStartY - topY; // 移动笔触到P3坐标

  context.moveTo(arrowX, arrowY); // 移动笔触到P1

  context.moveTo(mouseStartX, mouseStartY); // 绘制P1到P2的直线

  context.lineTo(mouseX, mouseY); // 计算P3的位置

  arrowX = mouseX + topX;
  arrowY = mouseY + topY; // 移动笔触到P3坐标

  context.moveTo(arrowX, arrowY); // 绘制P2到P3的斜线

  context.lineTo(mouseX, mouseY); // 计算P4的位置

  arrowX = mouseX + botX;
  arrowY = mouseY + botY; // 绘制P2到P4的斜线

  context.lineTo(arrowX, arrowY); // 上色

  context.strokeStyle = color;
  context.lineWidth = borderWidth; // 填充
...
function drawMosaic(mouseX, mouseY, size, degreeOfBlur, context) {
  // 获取鼠标经过区域的图片像素信息
  var imgData = context.getImageData(mouseX, mouseY, size, size); // 获取图像宽高

  var w = imgData.width;
  var h = imgData.height; // 等分图像宽高

  var stepW = w / degreeOfBlur;
  var stepH = h / degreeOfBlur; // 循环画布像素点

  for (var i = 0; i < stepH; i++) {
    for (var j = 0; j < stepW; j++) {
      // 随机获取一个小方格的随机颜色
      var color = getAxisColor(imgData, j * degreeOfBlur + Math.floor(Math.random() * degreeOfBlur), i * degreeOfBlur + Math.floor(Math.random() * degreeOfBlur)); // 循环小方格的像素点
...
function calculateToolLocation(position, toolWidth) {
  ...
  var mouseX = (position.width - toolWidth) / 2 + position.startX; // 工具栏Y轴坐标
...
var main_ScreenShort = /*#__PURE__*/function () {
  function ScreenShort(options) {
    ...
    this.cutOutBoxBorderArr = []; // 当前操作的边框节点

    this.borderOption = null; // 点击裁剪框时的鼠标坐标
    ...
    this.clickFlag = false; // 鼠标拖动状态

    this.dragFlag = false; // 单击截取屏启用状态

    this.clickCutFullScreen = false; // 上一个裁剪框坐标信息

    this.drawGraphPrevX = 0;
    this.drawGraphPrevY = 0;
    this.fontSize = 17; // 最大可撤销次数

    this.maxUndoNum = 15; // 马赛克涂抹区域大小

    this.degreeOfBlur = 5; // 截图容器位置信息
    ...
    this.screenShot = function (cancelCallback) {
      ...
      _this.startCapture(cancelCallback).then(function () {
        setTimeout(function () {
          ...
          if (context == null || _this.screenShortController == null) return; // 赋值截图区域canvas画布

          _this.screenShortCanvas = context; // 绘制蒙层
          ...
          var containerHeight = (_this$screenShortImag2 = _this.screenShortImageController) === null || _this$screenShortImag2 === void 0 ? void 0 : _this$screenShortImag2.height; // 用户有传宽高时，则使用用户的
          ...
          (_this$screenShortImag3 = _this.screenShortImageController.getContext("2d")) === null || _this$screenShortImag3 === void 0 ? void 0 : _this$screenShortImag3.drawImage(_this.videoController, 0, 0, containerWidth, containerHeight); // 存储屏幕截图

          _this.data.setScreenShortImageController(_this.screenShortImageController); // 添加监听
          ...
          (_this$screenShortCont4 = _this.screenShortController) === null || _this$screenShortCont4 === void 0 ? void 0 : _this$screenShortCont4.addEventListener("mouseup", _this.mouseUpEvent); // 停止捕捉屏幕
    ...
    this.mouseDownEvent = function (event) {
      ...
      var mouseY = nonNegativeData(event.offsetY); // 如果当前操作的是截图工具栏
      ...
      if (_this.data.getToolName() == "text" && _this.textInputController && _this.screenShortController && _this.screenShortCanvas) {
        // 修改鼠标样式
        _this.screenShortController.style.cursor = "text"; // 显示文本输入区域

        _this.data.setTextStatus(true); // 判断输入框位置是否变化
        ...
        if (_this.textInputPosition.mouseX != 0 && _this.textInputPosition.mouseY != 0 && _this.textInputPosition.mouseX != mouseX && _this.textInputPosition.mouseY != mouseY) {
          drawText(_this.textInputController.innerText, _this.textInputPosition.mouseX, _this.textInputPosition.mouseY, _this.data.getSelectedColor(), _this.fontSize, _this.screenShortCanvas); // 清空文本输入区域的内容

          _this.textInputController.innerHTML = ""; // 保存绘制记录
        ...
        var textMouseY = mouseY - 15 + _this.position.top; // 修改文本区域位置
        ...
        setTimeout(function () {
          ...
          if (_this.textInputController) {
            _this.textInputController.focus(); // 记录当前输入框位置
      ...
      if (_this.borderOption) {
        // 设置为拖动状态
        _this.data.setDraggingTrim(true); // 记录移动时的起始点坐标
        ...
        _this.drawGraphPrevY = _this.drawGraphPosition.startY; // 绘制裁剪框,记录当前鼠标开始坐标
    ...
    this.mouseMoveEvent = function (event) {
      ...
      if (!_this.data.getToolClickStatus() && _this.data.getDragging()) {
        // 修改拖动状态为true;
        _this.dragFlag = true; // 隐藏截图工具栏

        _this.data.setToolStatus(false);
      }

      _this.clickFlag = false; // 获取当前绘制中的工具位置信息
      ...
      var _this$drawGraphPositi = _this.drawGraphPosition,
          ...
          height = _this$drawGraphPositi.height; // 获取当前鼠标坐标

      var currentX = nonNegativeData(event.offsetX);
      var currentY = nonNegativeData(event.offsetY); // 绘制中工具的临时宽高

      var tempWidth = currentX - startX;
      var tempHeight = currentY - startY; // 工具栏绘制

      if (_this.data.getToolClickStatus() && _this.data.getDragging()) {
        // 获取裁剪框位置信息
        var cutBoxPosition = _this.data.getCutOutBoxPosition(); // 绘制中工具的起始x、y坐标不能小于裁剪框的起始坐标
        // 绘制中工具的起始x、y坐标不能大于裁剪框的结束标作
        // 当前鼠标的x坐标不能小于裁剪框起始x坐标，不能大于裁剪框的结束坐标
        // 当前鼠标的y坐标不能小于裁剪框起始y坐标，不能大于裁剪框的结束坐标


        if (!getDrawBoundaryStatus(startX, startY, cutBoxPosition) || !getDrawBoundaryStatus(currentX, currentY, cutBoxPosition)) return; // 当前操作的不是马赛克则显示最后一次画布绘制时的状态
      ...
      _this.operatingCutOutBox(currentX, currentY, startX, startY, width, height, _this.screenShortCanvas); // 如果鼠标未点击或者当前操作的是裁剪框都return


      if (!_this.data.getDragging() || _this.data.getDraggingTrim()) return; // 绘制裁剪框
    ...
    this.mouseUpEvent = function () {
      // 当前操作的是撤销
      if (_this.data.getToolName() == "undo") return; // 绘制结束

      _this.data.setDragging(false);

      _this.data.setDraggingTrim(false); // 截图容器判空
      ...
      if (!_this.dragFlag && !_this.clickCutFullScreen) {
        return;
      } // 调用者尚未拖拽生成选区
      // 鼠标尚未拖动
      // 单击截取屏幕状态为true
      ...
      } else if (!_this.dragFlag && !_this.data.getToolClickStatus()) {
        // 鼠标尚未拖动且工具栏未选择则不修改工具栏位置
        // 复原裁剪框的坐标
        _this.drawGraphPosition.startX = _this.drawGraphPrevX;
        _this.drawGraphPosition.startY = _this.drawGraphPrevY; // 显示截图工具栏
      ...
      _this.drawGraphPosition = _this.tempGraphPosition; // 如果工具栏未点击则保存裁剪框位置
      ...
      if (_this.screenShortController != null) {
        // 修改鼠标状态为拖动
        _this.screenShortController.style.cursor = "move"; // 显示截图工具栏

        _this.data.setToolStatus(true); // 复原拖动状态
        ...
        if (_this.toolController != null) {
          // 计算截图工具栏位置
          var toolLocation = calculateToolLocation(_this.drawGraphPosition, _this.toolController.offsetWidth); // 显示并设置截图工具栏位置
    ...
    var plugInParameters = new PlugInParameters_PlugInParameters(); // webrtc启用状态
    ...
    new CreateDom_CreateDom(options); // 读取参数中的画布宽高
    ...
    this.screenShortImageController = document.createElement("canvas"); // 实例化响应式data

    this.data = new InitData_InitData(); // 获取截图区域canvas容器
    ...
    if (screenShotContainer == null) return; // 调整层级
  ...
  _createClass(ScreenShort, [{
    ...
    value: function load(triggerCallback, cancelCallback) {
      ...
      var plugInParameters = new PlugInParameters_PlugInParameters();
      var canvasSize = plugInParameters.getCanvasSize(); // 设置截图区域canvas宽高

      this.data.setScreenShortInfo(window.innerWidth, window.innerHeight); // 设置截图容器位置

      this.data.setScreenShotPosition(this.position.left, this.position.top); // 设置截图图片存放容器宽高

      this.screenShortImageController.width = window.innerWidth;
      this.screenShortImageController.height = window.innerHeight; // 用户有传宽高则使用用户传进来的
      ...
      if (context == null) return; // 启用webrtc截屏时则修改容器宽高

      if (plugInParameters.getWebRtcStatus()) {
        // 设置为屏幕宽高
        this.data.setScreenShortInfo(window.screen.width, window.screen.height); // 设置为屏幕宽高

        this.screenShortImageController.width = window.screen.width;
        this.screenShortImageController.height = window.screen.height; // 用户有传宽高则使用用户传进来的
      ...
      if (!plugInParameters.getWebRtcStatus()) {
        ...
        html2canvas_default()(document.body, {}).then(function (canvas) {
          ...
          _this2.screenShortImageController = canvas; // 存储屏幕截图

          _this2.data.setScreenShortImageController(canvas); // 赋值截图区域canvas画布


          _this2.screenShortCanvas = context; // 绘制蒙层

          drawMasking(context); // 添加监听
    ...
    value: function operatingCutOutBox(currentX, currentY, startX, startY, width, height, context) {
      ...
      var _this$movePosition = this.movePosition,
          ...
          moveStartY = _this$movePosition.moveStartY; // 裁剪框边框节点事件存在且裁剪框未进行操作，则对鼠标样式进行修改

      if (this.cutOutBoxBorderArr.length > 0 && !this.data.getDraggingTrim()) {
        // 标识鼠标是否在裁剪框内
        var flag = false; // 判断鼠标位置

        context.beginPath();

        for (var i = 0; i < this.cutOutBoxBorderArr.length; i++) {
          context.rect(this.cutOutBoxBorderArr[i].x, this.cutOutBoxBorderArr[i].y, this.cutOutBoxBorderArr[i].width, this.cutOutBoxBorderArr[i].height); // 当前坐标点处于8个可操作点上，修改鼠标指针样式
        ...
        if (!flag) {
          // 鼠标移出裁剪框重置鼠标样式
          this.screenShortController.style.cursor = "default"; // 重置当前操作的边框节点为null
      ...
      if (this.data.getDraggingTrim()) {
        ...
        if (this.borderOption === 1) {
          // 计算要移动的x轴坐标
          var x = fixedData(currentX - (moveStartX - startX), width, this.screenShortController.width); // 计算要移动的y轴坐标

          var y = fixedData(currentY - (moveStartY - startY), height, this.screenShortController.height); // 重新绘制裁剪框
          ...
          var _zoomCutOutBoxPositio = zoomCutOutBoxPosition(currentX, currentY, startX, startY, width, height, this.borderOption),
              ...
              tempHeight = _zoomCutOutBoxPositio.tempHeight; // 绘制裁剪框
