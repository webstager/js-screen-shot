...
import { DEFAULT_MASK_COLOR, MIN_CROP_BOX_SIZE } from "@/lib/constants/cropBox";
...
export function drawCutOutBox(
  ...
  width = width !== 0 ? width : MIN_CROP_BOX_SIZE;
  height = height !== 0 ? height : MIN_CROP_BOX_SIZE;

  // 绘制蒙层
  context.save();
  const maskColor = userParamStore.maskColor;
  context.fillStyle = DEFAULT_MASK_COLOR;
  if (maskColor) {
